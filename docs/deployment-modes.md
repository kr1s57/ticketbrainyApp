...
as **bind-mounted files** from `proxy/Caddyfile` and `keycloak/apply-config.sh`.
When you run `git pull && docker compose pull && docker compose up -d`,
Compose **only recreates services whose image changed**. The `caddy:2` and
`curlimages/curl` images are stable and rarely change, so those two
containers **keep running with their previous in-memory config** even
if the bind-mounted files have been updated.

**Symptom of the gotcha:** your pentest / security checks show that the
new hardening (Caddy admin block, Keycloak master brute-force policy,
CORS whitelist) hasn't taken effect, even though the files on disk are
up to date.

**Fix:** always upgrade with `--force-recreate`:
...
The `--force-recreate` flag tells Compose to rebuild every service
container, not just the ones whose image changed, so the updated
bind-mounted config files are re-read on the next boot. Caddy reloads
its Caddyfile and `keycloak-init` re-runs `apply-config.sh` against
the live Keycloak instance.

Alternatively, restart the two bind-mount consumers individually:
...
> **⚠️ `WEB_BIND` configuration required** — v1.10.131 shipped a
> hardening fix (C-01) that binds the web container's exposed port
> on `127.0.0.1` by default, to prevent accidental public exposure
> of the cleartext HTTP service when the install runs with no
> reverse proxy. **This default does NOT work for `behind-waf`
> installs** — your WAF tourne sur une machine différente et arrive
> via le réseau LAN, donc elle ne peut pas contacter `127.0.0.1` du
> host Docker.
>
> **Required `.env` override** for this mode :
>
> ```
> # Option A — bind only on the LAN interface (recommended, most secure)
> WEB_BIND=10.x.y.z   # replace with the LAN IP your WAF connects to
>
> # Option B — bind on all interfaces (acceptable if the host has no
> # public IP, e.g. a machine in a DMZ with only LAN interfaces)
> WEB_BIND=0.0.0.0
> ```
>
> Without this override, your WAF receives 502 / Connection refused
> after every `docker compose up -d --force-recreate`. The symptom is
> "the site worked before the upgrade, now it's unreachable via the
> public domain but `curl http://127.0.0.1:${APP_PORT}` from the
> host still returns 200".
...
### Recommended posture — by order of preference

**If you have a VPN, bastion host, or private LAN access to your VPS**,
use it. Keycloak's admin console (`/admin/master/console/`, password
reset, realm management) should ideally **never be reachable from the
public Internet**. Put it behind a VPN, an SSH tunnel, or a corporate
bastion — that's the pattern large deployments use.

**If you don't have that infrastructure** (most self-hosted clients),
the TicketBrainy Keycloak admin IP allowlist is the pragmatic next
best thing. It restricts `/admin/*` and `/realms/master/*` to a
list of CIDRs you manage from the Settings → Security UI. All other
visitors get a hard **404** (not 403) so the admin console's existence
is hidden from scanners entirely.

**If neither is possible** (e.g. your admin IP is dynamic, you don't
have a fixed public IP), at minimum :
- Set a strong `KC_ADMIN_PASSWORD` (auto-generated by `install.sh` —
  don't change it to something weak)
- The v1.10.131 hardening applies Brute Force Protection on the
  master realm (5 failures → 15-min lockout, `length(14)` password
  policy) automatically, so credential-stuffing is neutralized
- Keep an eye on `docker logs aidesk-keycloak-1 | grep LOGIN_ERROR`
...
built-in Keycloak brute-force protection (5 failures → 15 min lockout
after the v1.10.131 hardening) and your strong `KC_ADMIN_PASSWORD`
prevent credential stuffing, but you may prefer to hide the admin UI
entirely from non-allowlisted IPs.
